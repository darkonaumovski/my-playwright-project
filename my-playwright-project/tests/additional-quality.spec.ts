...
test.describe("Additional quality coverage", () => {
  test("dismisses the menu with Escape and keeps focus in the menu", async ({
    login,
    header,
  }) => {
  ...
  test("keeps the inventory usable at a mobile viewport without horizontal overflow", async ({
    page,
    login,
    header,
    inventory,
  ...
  test("isolates carts between two browser contexts", async ({ browser }) => {
    ...
    try {
      ...
      await expect(inventoryOne.header.cartBadge).toHaveText("1");
      await expect(inventoryTwo.header.cartBadge).toHaveText("1");
      await inventoryOne.openCart();
      await inventoryTwo.openCart();
      await expect(
        pageOne.getByText("Sauce Labs Backpack", { exact: true }),
      ).toBeVisible();
      await expect(
        pageOne.getByText("Sauce Labs Bike Light", { exact: true }),
      ).toHaveCount(0);
      await expect(
        pageTwo.getByText("Sauce Labs Bike Light", { exact: true }),
      ).toBeVisible();
      await expect(
        pageTwo.getByText("Sauce Labs Backpack", { exact: true }),
      ).toHaveCount(0);
    } finally {
