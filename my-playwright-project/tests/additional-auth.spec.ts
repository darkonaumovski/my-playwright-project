...
test.describe('Additional authentication coverage', () => {
  test('guards every protected route when logged out', async ({ page, login }) => {
    ...
    for (const route of protectedRoutes) {
      ...
      await expect(login.errorMessage).toContainText(`You can only access '${route.split('?')[0]}' when you are logged in`);
    }
  });

  test('dismisses a login error and clears it after a successful retry', async ({ login }) => {
    await login.goto();
  ...
  test('supports keyboard login and masks the password', async ({ page, login }) => {
    await login.goto();
  ...
  for (const value of ['   ', 'standard_user_extra_long_value_'.repeat(10), '<script>alert(1)</script>', 'user/name?x=1']) {
    test(`handles unusual username input safely: ${value.slice(0, 18)}`, async ({ page, login }) => {
      await login.goto();
