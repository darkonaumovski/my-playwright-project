...
async function addBackpackAndOpenCheckout(
...
test.describe("Additional checkout coverage", () => {
  test("calculates multi-item subtotal, tax, and total numerically", async ({
    inventory,
    cart,
    checkout,
    header,
  ...
  test("retains checkout information while validating required fields", async ({
    inventory,
    cart,
    checkout,
  }) => {
  ...
  test("handles whitespace-only checkout data according to current demo behavior", async ({
    page,
    inventory,
    cart,
    checkout,
  ...
  test("cancels checkout without losing the cart", async ({
    page,
    inventory,
    cart,
    checkout,
  ...
  test("keeps completion idempotent and the cart empty after returning home", async ({
    page,
    inventory,
    cart,
    checkout,
  ...
  test("downloads a non-empty PDF with a valid PDF signature", async ({
    inventory,
    cart,
    checkout,
  }) => {
    ...
    assert(
      filePath !== null,
      "Playwright did not provide a path for the completed download.",
    );
